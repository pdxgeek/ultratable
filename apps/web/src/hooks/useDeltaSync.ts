import type { Fixture, Team, Venue } from '../db';
        ...
        async (seasonId: string) => {
            ...
            try {
                ...
                if (since) {
                    ...
                    const seasonFixtures = await db.fixtures
                        .where('seasonId')
                        .equals(seasonId)
                        .toArray();
                    ...
                    if (staleFixtures.length > 0) {
                        ...
                        const teamIds = new Set(
                            staleFixtures.flatMap((f) => [f.homeTeamId, f.awayTeamId]),
                        );
                        const teamRecords = await db.teams
                            .where('id')
                            .anyOf([...teamIds])
                            .toArray();
                        const teamName = (id: string) =>
                            teamRecords.find((t) => t.id === id)?.name || id.slice(0, 8);
                ...
                const result = await client
                    .query(
                        ...
                        staleRemediation ? { requestPolicy: 'network-only' } : {},
                    )
                    .toPromise();
                ...
                if (fixtures?.length > 0) {
                    ...
                    const stillStale = fixtures.filter(
                        (f: Fixture) =>
                            f.scheduledAt < now && !TERMINAL_STATUSES.includes(f.status),
                    );
                    if (stillStale.length > 0) {
                        console.log(
                            `[DeltaSync] ${syncLabel}: ${stillStale.length} STILL-STALE fixtures returned by server`,
                        );
                    }
                }
                ...
                await db.transaction(
                    'rw',
                    [db.teams, db.fixtures, db.venues, db.syncState],
                    async () => {
                        if (teams?.length > 0) {
                        ...
                        if (allUpdates.length > 0) {
                            const latest = allUpdates.reduce(
                                (max, cur) =>
