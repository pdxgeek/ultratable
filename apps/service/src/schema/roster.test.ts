...
describe('Team Roster', () => {
    ...
    it('should query teamRoster and return roster entries with player data', async () => {
        vi.mocked(repository.teams.getTeamRoster).mockResolvedValue([mockRosterEntry]);
        ...
        expect(repository.teams.getTeamRoster).toHaveBeenCalledWith('team-uuid-1', 'season-uuid-1');
    });
    ...
    it('should handle roster metadata with null values gracefully', async () => {
        ...
        vi.mocked(repository.teams.getTeamRoster).mockResolvedValue([entryWithNullMeta]);
    ...
    it('should handle importSquad with admin auth', async () => {
        ...
        vi.mocked(repository.teams.getTeamById).mockResolvedValue(mockTeam);
