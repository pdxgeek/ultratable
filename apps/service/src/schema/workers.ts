...
builder.objectType(JobRef, {
    fields: (t) => ({
        id: t.exposeString('id', {
            description:
                'Unique internal UUID for this job definition. Jobs track recurring or on-demand background tasks like fixture syncing.',
        }),
        name: t.exposeString('name', {
            description: 'Unique job identifier string (e.g. "sync-fixtures-40-2025").',
        }),
        scheduleCron: t.exposeString('scheduleCron', {
            nullable: true,
            description: 'Cron expression for recurring execution. Null for on-demand-only jobs.',
        }),
        isActive: t.exposeBoolean('isActive', {
            description: 'Whether this job is enabled for scheduled execution.',
        }),
        lastRunAt: t.expose('lastRunAt', {
            type: 'DateTime',
            nullable: true,
            description: 'Timestamp of the most recent execution start. Null if never run.',
        }),
        updatedAt: t.expose('updatedAt', {
            type: 'DateTime',
            description: 'ISO-8601 timestamp of the last update to the job record.',
        }),
    }),
});
...
builder.objectType(JobExecutionRef, {
    fields: (t) => ({
        id: t.exposeString('id', {
            description:
                'Unique internal UUID for this execution record. Each run of a job produces one execution with timing, status, and result metrics.',
        }),
        jobId: t.exposeString('jobId', {
            description:
                'UUID of the parent job that triggered this execution. Use this to filter executions for a specific job via the jobExecutions query.',
        }),
        status: t.exposeString('status', {
            description: 'Execution status: "running", "completed", or "failed".',
        }),
        startedAt: t.expose('startedAt', {
            type: 'DateTime',
            description: 'Timestamp when execution began.',
        }),
        finishedAt: t.expose('finishedAt', {
            type: 'DateTime',
            nullable: true,
        ...
        processedCount: t.exposeInt('processedCount', {
            nullable: true,
            description: 'Number of items successfully processed. Null if not tracked.',
        }),
        totalCount: t.exposeInt('totalCount', {
            nullable: true,
            description: 'Total number of items to process. Null if not known in advance.',
        }),
        apiCallsCount: t.exposeInt('apiCallsCount', {
            nullable: true,
            description: 'Number of external API calls made during execution. Null if not tracked.',
        }),
        updatedAt: t.expose('updatedAt', {
            type: 'DateTime',
...
const SystemLog = builder.simpleObject('SystemLog', {
    fields: (t) => ({
        ...
        module: t.string({
            description: 'Module that produced this log (e.g. "SyncEngine", "CacheService").',
        }),
        message: t.string({ description: 'Human-readable log message.' }),
        context: t.field({
            type: 'JSON',
            nullable: true,
            description: 'Additional structured data as JSON. Null if none.',
        }),
        createdAt: t.field({
            type: 'DateTime',
    ...
    t.field({
        ...
        args: {
            jobId: t.arg.string({
                required: false,
                description:
                    'Optional UUID of a specific job. When provided, only returns executions belonging to that job. Omit to see executions across all jobs.',
            }),
            limit: t.arg.int({
                required: false,
                description: `Maximum number of executions to return. Defaults to 50. Clamped to [1, ${MAX_PAGE_SIZE}].`,
            }),
        },
...
builder.queryField('systemLogs', (t) =>
    t.field({
        ...
        args: {
            limit: t.arg.int({
                required: false,
                description: `Maximum number of log entries to return. Defaults to 100. Clamped to [1, ${MAX_PAGE_SIZE}].`,
            }),
        },
    ...
    t.field({
        ...
        args: {
            name: t.arg.string({
                required: true,
                description:
                    'Unique job name that identifies the task to run (e.g. "sync-fixtures-40-2025"). For fixture syncs, the name encodes leagueSourceId and seasonYear.',
            }),
        },
                ...
                if (name.startsWith('sync-fixtures-')) {
                    ...
                    if (isNaN(leagueSourceId) || isNaN(seasonYear)) {
                        throw new GraphQLError(
                            `Invalid job name format: expected sync-fixtures-<leagueSourceId>-<seasonYear>, got "${name}"`,
                        );
                    }
                    const syncRes = await repository.fixtures.syncFixtures(
                        leagueSourceId,
                        seasonYear,
                        reporter,
                    );
