...
describe('PostgresFootballRepository - Formula & Graphics', () => {
    ...
    it('should save and retrieve ranking formulas', async () => {
        if (!db) return;
        const saved = await repository.leagues.saveRankingFormula(testFormula);
    ...
    it('should save and retrieve graphics', async () => {
        if (!db) return;
        const saved = await repository.graphics.saveGraphic(testGraphic);
    ...
    it('should upsert graphics on conflict', async () => {
        if (!db) return;
        await repository.graphics.saveGraphic(testGraphic);

        const updatedGraphic = { ...testGraphic, mimeType: 'image/jpeg' };
        const updated = await repository.graphics.saveGraphic(updatedGraphic);
    ...
    describe('Delta Sync (updatedAt filtering)', () => {
        it('should filter fixtures by updatedAt', async () => {
            ...
            const fixtures = await repository.fixtures.getFixtures(leagueSourceId, season);
