...
async function run() {
    try {
        ...
        console.log(
            `✅ Catalog sync complete. Processed ${syncResult.stats.processedCount} leagues across ${syncResult.stats.apiCallsCount} API calls.`,
        );
        ...
        if (!catLeague) {
            throw new Error(
                'Championship (League 40) not found in the catalog. Was sync successful?',
            );
        }
