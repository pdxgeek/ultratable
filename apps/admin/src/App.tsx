...
import type { Execution, Job } from './components/WorkersView';
                ...
                <header className="h-20 border-b border-slate-800/40 flex items-center justify-between px-12 backdrop-blur-md sticky top-0 z-10 bg-[#020617]/50">
                    <div>
                        <h1 className="text-sm font-medium text-slate-500 flex items-center gap-2">
                            ...
                            <span className="text-slate-200 capitalize underline decoration-sky-500/30 underline-offset-8 decoration-2 tracking-wide">
                                {activeTab}
                            </span>
                        </h1>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2 bg-slate-900/80 border border-slate-800/50 rounded-full px-4 py-1.5 shadow-inner">
                            ...
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.15em] italic">
                                System Normal
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 transition-all ease-out">
                        ...
                        {activeTab === 'api-keys' && (
                            <ApiKeyView
                                ...
                                currentKeyMasked={config?.apiFootballKeyMasked}
                            />
                        )}
                        {activeTab === 'database' && (
                            <DatabaseView config={config} onUpdate={fetchStatus} />
                        )}
                        {activeTab === 'leagues' && (
                            <LeaguesManagementView jobs={jobs} executions={executions} />
                        )}
                        {activeTab === 'workers' && (
                            <WorkersView
                                jobs={jobs}
                        ...
                        {activeTab === 'logs' && (
                            <LogsView logs={logs} onRefresh={fetchWorkerData} />
                        )}
                    </div>
                </div>
            </main>
