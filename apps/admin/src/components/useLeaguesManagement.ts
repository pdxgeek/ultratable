...
const logError = (label: string, e: unknown) =>
    console.error(`LeaguesManagementView: ${label} error:`, e);
...
export function useLeaguesManagement() {
    ...
    useEffect(() => {
        ...
        if (configTab === 'league' && league) {
            const config = league.configJson
                ? (JSON.parse(league.configJson) as Record<string, unknown>)
                : {};
        ...
        if (configTab === 'season' && season) {
            ...
            if (league) {
                api.fetchTeamsForSeason(season.id)
                    .then((data) => {
    ...
    const importSeason = async (leagueId: string, year: number) => {
        ...
        try {
            ...
            if (selectedCatalogLeagueId === leagueId)
                loadSeasons(leagueId, setSeasonsForCatalogLeague);
    ...
    const removeSeason = async (leagueId: string, seasonId: string, year: number) => {
        if (
            !window.confirm(
                `Are you sure you want to remove the ${year} season? This will delete all associated fixtures and standings data.`,
            )
        )
            return;
        setActionLoading(`${leagueId}-${year}`);
