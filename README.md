...
- **Upstream calls go through one rate-limited chokepoint.** Every call to api-football routes through `ApiFootballProvider.request(...)` and a per-instance `Bottleneck`. A response interceptor reads `X-RateLimit-Limit` and adapts the reservoir to the account's actual plan; 429s read `Retry-After` and queue retries. The asset CDN gets its own concurrency-only limiter at the consumer (`graphics.service.ts`) — same shape, different budget. Adding a new method on the provider that calls `this.client.get(...)` directly silently bypasses the budget. ([AI_README_FIRST.MD §7](AI_README_FIRST.MD#7-provider-rate-limiting))
- **Long syncs run on the workers pipeline, not the request thread.** Any mutation whose runtime scales with input size dispatches via `JobRunner.runInBackground` and returns a `JobExecution` row immediately; clients poll `jobExecutions` for completion. The 15-second authenticated GraphQL timeout is a structural ceiling, not a tunable. The closure-capture pattern (`let result; await JobRunner.run(...); return result`) doesn't survive background execution and is the smell that something's on the wrong side of the request boundary. ([AI_README_FIRST.MD §8](AI_README_FIRST.MD#8-workers--background-job-execution))

> [!IMPORTANT]
> The two newest contracts above — provider rate limiting and background-worker execution — together close the class of bug behind issue [#125](../../issues/125): a 20-team season import was timing out at the 15s ceiling because it ran inline AND had no rate-limit awareness for the upstream. The fix isn't a tuning knob; it's the architectural contract. Read [AI_README_FIRST.MD §7](AI_README_FIRST.MD#7-provider-rate-limiting) before adding a new upstream call and [§8](AI_README_FIRST.MD#8-workers--background-job-execution) before adding a mutation whose runtime depends on input size.
