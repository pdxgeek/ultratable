...
- **Workers are first-class.** Job + execution rows are real entities in the schema with their own resolvers, so the admin console doesn't need a side-channel — it queries the same GraphQL. Execution records carry `processedCount` / `totalCount` / `apiCallsCount` so I can see exactly how expensive an import was. Long-running syncs dispatch through `JobRunner.runInBackground` and return a `JobExecution` row immediately — the admin polls it instead of holding the GraphQL request open against a 15-second timeout. ([AI_README_FIRST.MD §8](AI_README_FIRST.MD#8-workers--background-job-execution))
- **Provider calls are rate-limited at the chokepoint.** Every upstream call to api-football routes through a single `Bottleneck`-wrapped `request()` method on the provider class. A response interceptor reads `X-RateLimit-Limit` off any response and resizes the limiter to match the account's actual plan (Free=10/min, Pro=300/min, …) — no env var, no probe call. 429s read `Retry-After` and queue a retry. The asset CDN gets its own (concurrency-only) limiter so a 20-team squad sync can't saturate the local HTTP socket pool. ([AI_README_FIRST.MD §7](AI_README_FIRST.MD#7-provider-rate-limiting))
