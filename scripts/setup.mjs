...
async function choose(question, options) {
    ...
    options.forEach((opt, i) => {
        stdout.write(`  ${c.cyan(`${i + 1})`)} ${c.bold(opt.label)}\n     ${c.dim(opt.description)}\n`);
    });
    while (true) {
...
async function main() {
    ...
    const dbMode = await choose(`How do you want to run Postgres? (current: ${c.cyan(inferredMode)})`, [
        { value: 'docker', label: 'Local Postgres via Docker Compose', description: 'Spins up a postgres:16-alpine container on :5432. Requires Docker. Storage features are disabled.' },
        { value: 'supabase', label: 'Supabase project', description: 'Use a hosted Supabase project. Provides Postgres + storage. Bring your own URL + keys.' },
        { value: 'system', label: 'I already have Postgres running', description: 'You provide the DATABASE_URL. Storage features are disabled unless you also supply Supabase keys.' },
    ]);
    ...
    if (dbMode === 'supabase') {
        ...
        vars.SUPABASE_URL = await ask('Supabase project URL (https://xxx.supabase.co)', { def: existing.SUPABASE_URL });
        vars.SUPABASE_ANON_KEY = await ask('Supabase anon / publishable key', { def: existing.SUPABASE_ANON_KEY, secret: true });
        vars.SUPABASE_SERVICE_ROLE_KEY = await ask('Supabase service role key', { def: existing.SUPABASE_SERVICE_ROLE_KEY, secret: true });
        ...
        if (!docker) {
            stdout.write(c.red('  Docker is not installed — pick a different mode or install Docker first.\n'));
            process.exit(1);
        }
        ...
        if (await confirm('Also configure Supabase keys for storage features?', Boolean(existing.SUPABASE_URL))) {
            vars.SUPABASE_URL = await ask('Supabase project URL', { def: existing.SUPABASE_URL });
            vars.SUPABASE_ANON_KEY = await ask('Supabase anon key', { def: existing.SUPABASE_ANON_KEY, secret: true });
            vars.SUPABASE_SERVICE_ROLE_KEY = await ask('Supabase service role key', { def: existing.SUPABASE_SERVICE_ROLE_KEY, secret: true });
        }
    }
    ...
    vars.API_FOOTBALL_KEY = await ask('API_FOOTBALL_KEY (blank to skip)', { def: existing.API_FOOTBALL_KEY, secret: true });
    if (!vars.API_FOOTBALL_KEY) {
        stdout.write(c.yellow('  ⚠  No API-Football key — sport-data endpoints will fail until you add one.\n'));
    }

    // ── Auth ─────────────────────────────────────────────────
    header('Authentication');
    if (existing.BETTER_AUTH_SECRET) {
        stdout.write(`  Reusing existing BETTER_AUTH_SECRET (${mask(existing.BETTER_AUTH_SECRET)}).\n`);
    } else {
    ...
    if (existsSync(SERVICE_ENV)) {
        ...
        stdout.write(c.dim(`  Backed up existing apps/service/.env → ${path.relative(ROOT, backupPath)}\n`));
    }
    ...
    if (dbMode === 'docker') {
        ...
        if (await confirm('Start the local Postgres container now (docker compose up -d postgres)?', true)) {
    ...
    if (await confirm(`Run \`npm install\` at the workspace root${nodeModulesExists ? ' (already present — re-install)' : ''}?`, !nodeModulesExists)) {
        if (!run('npm', ['install'])) {
    ...
    if (await confirm('Run `npm run db:push` in apps/service now?', true)) {
        if (!run('npm', ['run', 'db:push'], { cwd: path.join(ROOT, 'apps/service') })) {
            stdout.write(c.red('  Migrations failed — check that Postgres is reachable and re-run `npm run db:push --prefix apps/service`.\n'));
        } else {
